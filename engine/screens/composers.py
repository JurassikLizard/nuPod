"""
Music/Composers screen — browse music by composer.

Selecting a composer shows their works (songs attributed to them).
...
# Stub composers with song indices
STUB_COMPOSERS = [
    {"name": "Ludwig Van",         "songs": [0, 13, 19]},
    {"name": "Wolfgang A.",        "songs": [1, 15, 20]},
    {"name": "Clara Harmonic",     "songs": [2, 7, 12]},
    {"name": "Miles Electric",     "songs": [3, 10, 17]},
    {"name": "Elena Strings",      "songs": [4, 8, 14]},
    {"name": "Duke Silver",        "songs": [5, 11, 16]},
    {"name": "Yuki Winds",         "songs": [6, 9, 18]},
]


class ComposersScreen(Screen):
    """Music > Composers: composer list with drill-down to works."""
    ...
    def handle_input(self, event, state):
        ...
        if isinstance(event, ButtonPress):
            ...
            if btn == Button.CENTER:
                if not self._stack:
                    composer = STUB_COMPOSERS[self._selected]
                    songs = [(i, STUB_SONGS[i]) for i in composer["songs"]]
                    self._stack.append({"label": composer["name"], "items": songs})
                    self._selected = 0
                    self._scroll_offset = 0
                else:
