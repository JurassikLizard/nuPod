"""
Music/Genres screen — browse music by genre.

Selecting a genre shows songs in that genre (or artists/albums for it).
...
# Stub genres with song indices
STUB_GENRES = [
    {"name": "Rock",       "songs": [0, 1, 2, 14, 15, 16]},
    {"name": "Pop",        "songs": [3, 4, 5, 17, 18]},
    {"name": "Electronic", "songs": [6, 7, 8, 9]},
    {"name": "Jazz",       "songs": [10, 11, 12]},
    {"name": "Classical",  "songs": [13, 19]},
    {"name": "Indie",      "songs": [0, 3, 6, 10, 14, 18]},
    {"name": "Ambient",    "songs": [7, 8, 9, 12, 13]},
    {"name": "Folk",       "songs": [1, 4, 11, 15, 19]},
    {"name": "Blues",      "songs": [2, 5, 16, 17]},
    {"name": "R&B",        "songs": [0, 4, 8, 12, 18, 20]},
]


class GenresScreen(Screen):
    """Music > Genres: genre list with drill-down to songs."""
    ...
    def handle_input(self, event):
        ...
        if isinstance(event, ButtonPress):
            ...
            if btn == Button.CENTER:
                if not self._stack:
                    genre = STUB_GENRES[self._selected]
                    songs = [(i, STUB_SONGS[i]) for i in genre["songs"]]
                    self._stack.append({"label": genre["name"], "items": songs})
                    self._selected = 0
                    self._scroll_offset = 0
                else:
